...
for ((j=0; j<"${#QUERIES[@]}"; j++)); do
	...
	for ((i=0; i<"${#DATABASE[@]}"; i++)); do
		...
		blastp -query $PWD/$ENTRY/${QUERIES[j]}.seq.fa -db $PWD/genomes/${DATABASE[i]} -max_target_seqs ${SEQS[i]} -max_hsps 1 -outfmt "6 sseqid"  -out $PWD/$ENTRY/${QUERIES[j]}.${DATABASE[i]}.seq.tblastn
		
		#outputs the FULL tblastn results (outfmt 1).  You can scan this file to gauge completeness based on e-values, etc
		blastp -query $PWD/$ENTRY/${QUERIES[j]}.seq.fa -db $PWD/genomes/${DATABASE[i]} -max_hsps 1 -outfmt 1  -out $PWD/$ENTRY/${QUERIES[j]}.${DATABASE[i]}.seq.tblastn.full
	...
	#Pull the full length protein sequences from the blast databases using blastdbcmd (part of the BLAST package) and remove_stop.py
		...
		python $PWD/scripts/remove_stop.py $PWD/$ENTRY/${QUERIES[j]}.${DATABASE[i]}.seq.tblastn.blastdb.stop.fa $PWD/$ENTRY/${QUERIES[j]}.${DATABASE[i]}.seq.tblastn.blastdb.translate.fa
		...
		python $PWD/scripts/pull_id_fasta_coding.py $ENTRY ${QUERIES[j]} ${DATABASE[i]} ${HEADER[i]}
	done
