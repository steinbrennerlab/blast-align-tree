...
for ((i=0; i<"${#QUERIES[@]}"; i++)); do
	if ((${#SLICE[@]}>0)); then
		python $PWD/scripts/extract_translate2.py $ENTRY ${QUERIES[i]} ${QUERY_DATABASES[i]} ${SLICE[0]} ${SLICE[1]}
	else
		python $PWD/scripts/extract_translate2.py $ENTRY ${QUERIES[i]} ${QUERY_DATABASES[i]}
	fi
done
...
for ((j=0; j<"${#QUERIES[@]}"; j++)); do
	...
	#Pull the full length CDS sequences from the blast databases using blastdbcmd (part of the BLAST package) and remove_stop.py
		...
		python $PWD/scripts/translate_db2.py $ENTRY ${QUERIES[j]} ${DATABASE[i]}
		...
		python $PWD/scripts/pull_id_fasta2.py $ENTRY ${QUERIES[j]} ${DATABASE[i]} ${HEADER[i]} .seq.tblastn.blastdb.fa
		python $PWD/scripts/pull_id_fasta2.py $ENTRY ${QUERIES[j]} ${DATABASE[i]} ${HEADER[i]} .seq.tblastn.blastdb.translate.fa


		#Creates coding files, these list of genes with the genome they originally came from

		python $PWD/scripts/pull_id_fasta_coding2.py $ENTRY ${QUERIES[j]} ${DATABASE[i]} ${HEADER[i]}
	done
